...
<div class="col-md-12 grid-margin stretch-card formular">
  <div class="card">
    <div class="card-body">
        <h4 class="card-title">
          Show & Edit The freelance for :  <span style="background: #e5e5e5; padding: 5px 10px; border-radius: 5px">{{ $cli }}</span>
        ...
        <form method="POST" action="{{ route('freelance_updated_admin', [ 'ref' => $item->ref ]) }}" enctype="multipart/form-data">
        {{ csrf_field() }}
        ...
        <a title="Delete" href="{{ route('freelance_deleted_admin',[ 'ref' => $item->ref ]) }}" ><button class="btn btn-danger btn-icon-text btn-md" type="button" onclick="return confirm('Are you sure to delete this item ?');"><i class=" mdi mdi-times  btn-icon-prepend"></i> Delete</button></a>
      </form>
    </div>
  </div>
</div>
