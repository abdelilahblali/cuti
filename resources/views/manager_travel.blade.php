...
<div class="col-lg-12 grid-margin stretch-card">
  <div class="card">
    <div class="card-body">
      <h4 class="card-title">Business Trip</h4>
        ...
        <table id="table2" class="table display nowrap" style="width: 100%;">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="bold"><?php echo date('m/d/Y', strtotime($item->from_date)); ?></td>
              <td class="bold"><?php echo date('m/d/Y', strtotime($item->to_date)); ?></td>
              <?php 
              ...
              <td>
                <a title="View" href="{{ route('manager_travel_show',[ 'ref' => $item->ref ]) }}" ><button class="btn btn-sm btn-success" type="button">View</button></a>
                <a title="Delete" href="{{ route('manager_travel_delete',[ 'ref' => $item->ref ]) }}"  onclick="return confirm('Are you sure to delete this element ?');"><button class="btn btn-sm btn-danger" type="button">Delete</button></a>
                @if($item->act==1)
                <a title="Print" href="{{ route('travelbusiness_form',[ 'ref' => $item->ref ]) }}" target="_blank"><button class="btn btn-sm btn-warning" type="button">Print</button></a>
                @endif
              </td>
            </tr>
            @endforeach
